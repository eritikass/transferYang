(function () {
     ...
     * @type {string}
     */
    var CURRENCY_FEED = 'api.fixer.io';
...
//    "www.amazon.co.jp": "japan",
    ...
    function getRates(callback) {
        ...
        if (CURRENCY_FEED == 'api.fixer.io') {
            var callback_name = "yang_apifixerio_" + $.now();
            chrome.runtime.sendMessage({"message": "yang_fixerdata_push", "callback_name": callback_name});
            chrome.runtime.onMessage.addListener(
                function (request) {
                    if (request.message === callback_name) {
                        rates = request.data || {};
                        callback();
                    }
                }
    ...
    function showDialogWithData() {
        ...
        $.each(displayList, function (index, pricedata) {
            ...
            var htmlTb = '<tr class="pricechecked_amazon_off' + index + '">'
                ...
                + '<td class="pricechecked_amazon_rr4 pricechecked_amazon_numbcol ' + (priceDif < 0 ? 'pricechecked_amazon_numbcolpos' : (priceDif > 0 ? 'pricechecked_amazon_numbcolneg' : '')) + '">' + formatNumber(Math.abs(priceDif)) + '</td>'
                + '</tr>';
...
//            +'</td>'
        ...
        chrome.runtime.sendMessage({"message": "yang_push_version"});
    }
    ...
    chrome.runtime.onMessage.addListener(
        function (request) {
            if (request.message === "yang_display_version") {
                $('.pricechecked_amazon_plugin_version').html(', v' + request.version);
            }
        }
    );
